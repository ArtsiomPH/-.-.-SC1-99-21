...
class Add_medcine(ModelForm):
    ...
    general_info = forms.CharField(min_length=10, label="Информация", required=True, validators=[RegexValidator(
        regex='[а-яёА-Я0-9 .,!-]+$'
                                   ...
                                   help_text="Ввод кириллицей с большой буквы",
...
class Add_synonyms(ModelForm):
    comm_name = forms.CharField(max_length=50, label="Торговое наименование",
                                validators=[RegexValidator(regex='^[А-Я][а-яёА-я0-9 -]+$')],
                                error_messages={'invalid': 'Введите название кирилицей с большой буквы'},
                                help_text="Ввод кирилицей с большой буквы",
                                widget=forms.TextInput(attrs={'autocomplete': 'off'}))
    url_name = forms.CharField(max_length=50, label="Наименование на английском",
                               validators=[RegexValidator(regex='^[a-z][a-zA-z -]+$')],
                               error_messages={'invalid': 'Введите наименование латиницей с маленькой буквы'},
                               help_text="Ввод латиницей с маленькой буквы",
                               widget=forms.TextInput(attrs={'autocomplete': 'off'}))
...
class Add_literature(ModelForm):
    source_name = forms.CharField(min_length=10, label="Источник", required=True,
                                  widget=forms.TextInput(attrs={'autocomplete': 'off'}),
                                  error_messages={'blank': 'Поле обязательно для заполнения',
